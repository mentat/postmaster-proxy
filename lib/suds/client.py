...
class Client(object):
    ...
    def __init__(self, url, **kwargs):
        ...
        if url not in wsdls:
            ...
            wsdls[url] = reader.open(url)

        self.wsdl = wsdls[url]
        self.wsdl.options = options
...
class Method:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if not self.faults():
            try:
                return client.invoke(args, kwargs, async=async)
            except WebFault, e:
                return (500, e)
        else:
            return client.invoke(args, kwargs, async=async)
...
class SoapClient:
    ...
    def invoke(self, args, kwargs, async):
        """
        ...
        timer.start()
        if async:
            future = self.send_async(soapenv)
            result = future
        else:
